...
export Person operator""_p(const char* s, size_t)
{
	...
	// The following extraction operator will stop at the first white space,
	// which is exactly what we need to extract the first name, because the
	// first name is not allowed to contain spaces.
	input >> firstName;

	// After the first name, there is at least one space in the name.
	// The following statement consumes all whitespace at the current
	// position in the strema;
	input >> std::ws;
